...
Learn, on the Unassigned column, goes further for the channels the app knows too little about: it looks at three recent videos of each, one request at a time, never while subscriptions refresh, and it stops the moment YouTube pushes back. It carries on in the background as the suggestions fill in, and a second press takes up where it stopped. What it learns is kept with the channel.
