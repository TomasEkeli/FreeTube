...
Make a profile with New profile at the end of the strip and type its name right there. Right-click a bubble to rename it or change its colour. Drag bubbles to put the profiles in your own order, which every list of profiles in the app follows, or move the focused one with Ctrl+Shift+Left and Right. New profiles go at the end.

Profile settings still handle a profile's default and deletion. The channel lists there are gone.
